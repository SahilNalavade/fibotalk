...
const handleSave = useCallback(async () => {
  ...
  // If this database is being activated, deactivate other active databases
  if (isEnabled) {
    try {
      // Fetch all databases to find active ones
      ...
      const activeDatabases = response.data.records.filter(
        (record) => record.fields.State === 'Active'
      );

      if (activeDatabases.length > 0) {
        // Deactivate other active databases
        const deactivateRequests = activeDatabases.map((record) =>
          axios.patch(
            `https://api.airtable.com/v0/app4ZQ9jav2XzNIv9/DatabaseConfig`,
            {
              records: [
      ...
      console.error('Error deactivating other databases:', error);
      toast({
        title: 'Error',
        description: 'Failed to deactivate other active databases.',
        status: 'error',
        duration: 5000,
        isClosable: true,
        position: 'top-right',
  ...
  try {
    // Save the new or edited configuration to Airtable
    ...
    if (airtableResponse.status === 200) {
      ...
      // Trigger the callback to refresh the data in TableComponent
      if (onSaveSuccess) {
        onSaveSuccess();
      }
      onBack();
        ...
        {activeStep === 1 && formData.databaseServer === 'SnowFlake' && (
          ...
          <VStack spacing={6} align="stretch">
            ...
            <HStack spacing={6}>
              ...
              <FormControl id="projectId" isRequired>
